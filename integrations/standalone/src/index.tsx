...
import { App, ClientContextProvider, initQueryClient, PersistenceClientJsonRpc, QueryProvider } from '@axonivy/persistence-editor';
...
export async function start() {
  ...
  const initialize = async (connection: Connection) => {
    const client = await PersistenceClientJsonRpc.startClient(connection);
  ...
  const reconnect = async (connection: Connection, oldClient: PersistenceClientJsonRpc) => {
    await oldClient.stop();
    return initialize(connection);
  };

  await webSocketConnection<PersistenceClientJsonRpc>(PersistenceClientJsonRpc.webSocketUrl(server)).listen({
